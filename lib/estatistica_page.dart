...
//////////MAP COM POSIÇAO POSIÇAO INDEX DOS INDICADORES/STOCK
  ...
  void initState() {
    ...
    setState(() {
      ...
      //   // print("hashcode:$hashCode");
         ...
         _saveFirebaseEstatistica("initState"); 
  ...
  _saveFirebaseEstatistica(type) async {
    query = FirebaseFirestore.instance.collection('$stockname');
    appState = StateWidget.of(context).state;
    List<Map> listbuysignals = buysignals
        .map((e) {

              if(e.id == 13){

                  List<Map> listmulti = e.indicadoresopcoes.map((e) => {
                    "id": e.id,
                    "name": e.name,
                    "description": e.description,
                    "configselecionado": {
                      "id": e.configselecionado.id,
                      "title": e.configselecionado.title,
                    },
                    "parametro": e.parametro}).toList();
                    return {
                          "id": e.id,
                          "name": e.name,
                          "description": e.description,
                          "indicadoresopcao":listmulti,
    ...
    List<Map> listsellsignals = sellsignals
        .map((e) {
    ...
    Map mapestatistica = {
      ...
      "gain": FunctionStock.roundDouble(estatistica.gain, 2),
      "loss": FunctionStock.roundDouble(estatistica.loss, 2),
      "qtddiasgain": estatistica.qtddiasgain,
      "qtddiasloss": estatistica.qtddiasloss,
      "qtdgain": estatistica.qtdgain,
      "qtdloss": estatistica.qtdloss,
      "payoff": estatistica.payoff,
      "total": estatistica.total,
      "dadosanalisados":stocks.length,
      "profitporcentagem": estatistica.profitporcentagem,
      "maximumdrawndown": estatistica.maximumdrawndown,

      
    };

    try {
      query.firestore.collection("backtest").doc().set({
        "name": stockname,
        "buysignals": listbuysignals,
        "sellsignals": listsellsignals,
        "estatistica": mapestatistica,
        "dtinicio": tradesFiltered[0].entrada.datetime,
        "dtfim": tradesFiltered.last.saida.datetime,
  ...
  Estatistica generateEstatistica() {
    ...
    if (tradesFiltered.isNotEmpty) {
      tradesFiltered.forEach((e) {
        ...
        if (precoVenda > precoEntrada) {
          ...
          estatistica.profitporcentagem += ((precoVenda - precoEntrada) / precoEntrada) * 100;
        } else {
          //////////Losssssss///////
    ...
    if(estatistica.gain == 0 && estatistica.loss == 0){
      ...
      estatistica.payoff = FunctionStock.roundDouble((estatistica.gain / 1), 2);
    }else if(estatistica.gain == 0 && estatistica.loss > 0){
      estatistica.payoff = 0;
    }else{
      estatistica.payoff = FunctionStock.roundDouble((estatistica.gain / estatistica.loss), 2);
      estatistica.payoff =  estatistica.payoff * -1;
    }
    estatistica.maximumdrawndown = FunctionStock.roundDouble(maximumdrawndown, 2);
            ...
            Column(children: [
                  Text("Resultado",
                        style: TextStyle(
                            ...
                            fontWeight: FontWeight.w500,
                            color: AppColors.white)),
                    Text("${FunctionStock.roundDouble(estatistica.profitporcentagem, 2)}%",
                        style: TextStyle(
                            fontSize: 20,
                            fontWeight: FontWeight.w500,
                            color: AppColors.white)),
