...
class ForcaPage extends StatefulWidget {
  ...
  @override
  _ForcaPageState createState() => _ForcaPageState(stockname: stockname,stocks: stocks);
}
...
class _ForcaPageState extends State<ForcaPage> {
 ...
 @override
  void initState() {
    ...
    setState(() {
      selectedRange = RangeValues(stocks[0].datetime.year.roundToDouble(), stocks.last.datetime.year.roundToDouble());
...
double roundDouble(double value, int places){ 
  ...
  Map<String,List<SazonalidadeForca>> generateDiasFortes() {
    ...
    12: 0.0,};
     ...
     try { 
      ...
      print("anos $rangeano");
       ...
       for (int i = 0; i <= rangeano; i++) {
          ...
          List<Stock> filteredporano = candles.where((e) => e.datetime.year == ano).toList();
          for (int j = 1; j <= 12; j++) {

            List<Stock> filteredpormes = filteredporano.where((e) => e.datetime.month == j).toList();
     ...
     Map<String,List<SazonalidadeForca>> sazonalidade = {"sazonalidadenegativas":sazonalidadenegativalist,"sazonalidadepositivas":sazonalidadepositivalist};
          ...
          child: SfCartesianChart(
                        ...
                        series: <ChartSeries>[
                            AreaSeries<SazonalidadeForca, dynamic>(
                                ...
                                yValueMapper: (SazonalidadeForca sazonal, _) => sazonal.qtddias
                            ),
...
_buildPageMain(BuildContext context){
              ...
              return Column(
                ...
                children: [
                    Text("Soma dos dias positivos de cada mês respectivo pelo intervalo de datas",textAlign: TextAlign.center ,style: TextStyle(color: Colors.white),),
                    Text("Range",textAlign: TextAlign.center ,style: TextStyle(color: Colors.white,fontSize: 20),),
                  RangeSlider(
                    ...
                    onChanged: (RangeValues value) { setState(() {
                          selectedRange = RangeValues(value.start.roundToDouble(),value.end.roundToDouble());
                           candles = stocks.where((e) => e.datetime.year >= selectedRange.start.toInt() &&  e.datetime.year <= selectedRange.end.toInt() ).toList();
