...
class FunctionStock {
  ...
  static Future<List<Stock>> parseJsonFrom(String assetsPath) async {
    ...
    List<Stock> stockslist = json
      ...
      List<Stock> stockslistfiltered = stockslist
                      .where((e) =>
                          e.datetime.year >= 2000 
                      )
                      .toList();
      return stockslistfiltered;
  }

   static Future<List<StockName>> parseJsonNameFromAssets(String assetsPath) async {
    print('--- Parse json from: $assetsPath');
    String stringjson = await rootBundle.loadString(assetsPath);
    List<dynamic> json = jsonDecode(stringjson);
    List<StockName> stockslist = json
          .map(
            (dynamic item){
...
static candleInfo(List<Stock> stocks, int index, num emaBodyValue, num c_Len){
    ...
    var c_BlackBody = open > close;
     ...
     var c_ShadowEquals = c_UpShadow == c_DnShadow || ((c_UpShadow - c_DnShadow).abs() / c_DnShadow * 100) < c_ShadowEqualsPercent && ((c_DnShadow - c_UpShadow).abs() / c_UpShadow * 100) < c_ShadowEqualsPercent;
     var c_IsDojiBody = c_Range > 0 && c_Body <= c_Range * c_DojiBodyPercent / 100;
