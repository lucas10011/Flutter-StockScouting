  ...
  static atr(num period, List<Stock> stocks) {
    ...
    for (int i = 0; i < stocks.length; i++) {
        ...
        if (i >= period && period > 0) {
          ...
          point = DataPoint(x:stocks[i].datetime,y:average,stock:stocks[i], index: i);
          points.add(point);
          ...
          if (i == period - 1) {
            ...
            point = DataPoint(x:stocks[i].datetime,y:0,stock:stocks[i],index: i);
            points.add(point);
          }
            
        }
        temp.add(DataPoint(x:stocks[i].datetime,y:average,stock: stocks[i],index: i));
...
static List<DataPoint> ema(int period, List<Stock> stocks) {
    ...
    if (stocks.length >= period && period > 0) {
      ...
      while (indexwhile < stocks.length) {
         ...
         final num prevAverage = points[indexwhile - 1].y;
          final num yValue = (stocks[indexwhile].close - prevAverage) * k + prevAverage;
          point = DataPoint(x:stocks[indexwhile].datetime,y:yValue,stock:stocks[indexwhile],index:indexwhile);
          points.add(point);
        indexwhile++;
      }
    }
  ...
  static List<num> emabody(int period, List<Stock> stocks) {
    ...
    if (stocks.length >= period && period > 0) {
      ...
      while (indexwhile < stocks.length) {
        ...
        var c_BodyHi = math.max(stocks[indexwhile].close, stocks[indexwhile].open);
        var c_BodyLo = math.min(stocks[indexwhile].close, stocks[indexwhile].open);
        var c_Body = c_BodyHi - c_BodyLo;
  ...
  //////////////////////////////////Fim calculo Média Exponencial/////////////////////////////////////////
 
 //////////////////////////////////Inicio calculo AVERAGE TRUE RANGE/////////////////////////////////////////
...
//   }
 ...
 //////////////////////////////////Fim calculo AVERAGE TRUE RANGE/////////////////////////////////////////
  ...
  static accumulationdistribution(List<Stock> stocks) {
    ...
    for (i = 0; i < stocks.length; i++) {
      ...
      sum = (sum + value * stocks[i].volume).isNaN ? sum : sum + value * stocks[i].volume;
      point = DataPoint(x:stocks[i].datetime, y:sum.toInt(),stock:stocks[i],index: i);
      points.add(point);
...
//////////////////////////////////Inicio calculo Bollinger/////////////////////////////////////////////
 static List<BollingerPointData> bollinger(int desvio, int periodo, List<Stock> stocks) {
    //prepare data
...
//////////////////////////////////Inicio calculo MACD/////////////////////////////////////////////
  ...
  static Map macd(num fastPeriod,
      num slowPeriod,
      num period,
      List<Stock> stocks,) {
    ...
    if (stocks.isNotEmpty &&
      ...
      final List<num> shortEMA = _calculateEMAValues(
          slowPeriod, stocks, 'close');
          // print("/////////////////////////SHORT EMA");
      final List<num> longEMA = _calculateEMAValues(
          fastPeriod, stocks, 'close');
          //  print("/////////////////////////longEMA EMA");
      final List<num> macdValues = _getMACDVales(fastPeriod, slowPeriod, shortEMA, longEMA);
      macdpoints = _getMACDPoints(
          macdValues,
          stocks,
          fastPeriod
         );
      ...
      histogrampoints = _getHistogramPoints(fastPeriod,period,macdValues, signalEMA, stocks);
    
  
     
      for(int i = 0;i < fastPeriod - 1; i++){

        macdpoints.insert(0, DataPoint(x:stocks[i].datetime,y: null,stock: stocks[i],index:i));

      }
      for(int i = 0;i < (fastPeriod + period - 2); i++){

        signalpoints.insert(0, DataPoint(x:stocks[i].datetime,y: null,stock: stocks[i],index:i));

      }
      for(int i = 0;i < (fastPeriod + period - 2); i++){

        histogrampoints.insert(0, DataPoint(x:stocks[i].datetime,y: null,stock: stocks[i],index:i));
      ...
      // //  print("${stocks[i].datetime}:::::::::MACDPOINT:${macdpoints[i].y}///SIGNALPOINT:${signalpoints[i].y}////HISTOGRAMPOINT:${histogrampoints[i].y}");
      ...
      return {"macdCollection":macdpoints,"signalCollection":signalpoints,"histogramCollection":histogrampoints,};
...
static num _getFieldValue(
      List<dynamic> point, int itr, String valueField) {
    num val;
    if (valueField == 'low') {
      val = point[itr]?.low;
 ...
 static List<num> _calculateEMAValues(
  ...
  static List<DataPoint> _getMACDPoints(
    ...
    while (dataMACDIndexwhile < stocks.length) {
      ...
      //     ));  
      //   }else{
      //     macdCollection.add(DataPoint(x:stocks[i].datetime,y:null,stock:stocks[i],index:i));
      //   }
  ...
  static List<DataPoint> _getSignalPoints(
    ...
    // final num len = fastPeriod + period - 2;
    //  /////i - len = posiçao 0 do index
    //   for (int i = 0; i < stocks.length; i++) {
    //     if (!(i < len)) {
    ...
    //     }else{
    //       signalCollection.add(DataPoint(x:null,y:null,stock:null,index:null));
    //     }
  ...
  static List<num> _getMACDVales(fastPeriod, slowPeriod,
  ...
  static List<DataPoint> _getHistogramPoints(
    ...
    //       histogramCollection.add(DataPoint(
    //       x:stocks[i].datetime,
    //       y:macdPoints[(i - len) + (period - 1)] -  signalEma[i - len],
    //       stock:stocks[i],
    //       index: i
    //       ));
          
    //     }else{
    //       histogramCollection.add(DataPoint(x:null,y:null,stock:null,index:null));
    //     }
       
    //   }
   
    // histogramCollectionwhile.forEach((e) => print("${e.x} => ${e.y}"));
    // print("//////////////");
    // histogramCollection.forEach((e) => print("${e.x} => ${e.y}"));
  ...
  static List<dynamic> ifr(int period, List<Stock> stocks) {
    ...
    if (stocks.isNotEmpty &&
      ...
      num loss = 0;
        ...
        signalValues.add(DataPoint(x:stocks[0].datetime,y:0,stock:stocks[0],index: 0));
      ...
      for (int i = 1; i <= period; i++) {
        ...
        if(i != period){
           signalValues.add(DataPoint(x:stocks[i].datetime,y:0,stock:stocks[i],index: i));
        }
      }
          ...
          gain = gain / period;
          loss = loss / period;
          signalValues.add(DataPoint(x:stocks[period].datetime,y:100 - (100 / (1 + (gain / loss))),stock:stocks[period],index: period));
      ...
      for (int j = period + 1; j < stocks.length; j++) {
          ...
          if (close > prevClose) {
            gain = (gain * (period - 1) + (close - prevClose)) /
                period;
            loss = (loss * (period - 1)) / period;
          } else if (close < prevClose) {
            loss = (loss * (period - 1) + (prevClose - close)) /
                period;
...
//////////////////////////////////Inicio calculo Estocastico/////////////////////////////////////////////
  ...
  static List<DataPoint> stochasticCalculation(
    ...
    if (data.length >= period + kPeriod && kPeriod > 0) {
      ...
      for (int i = 0; i < data.length; i++) {
        if (!(i < len)) {
          pointCollection.add(DataPoint(x:data[i].x,y:values[i - len],stock:data[i].stock,index:i));
          data[i].y = values[i - len];
        }else{
          pointCollection.add(DataPoint(x:data[i].x,y:null,stock:data[i].stock,index:i));
        }
      }
    }
 ...
 static List<DataPoint> calculatePeriod(
    ...
    final List<DataPoint> modifiedSource =
        <DataPoint>[];
    ...
    if (data.length > period) {
      ...
      for (int i = 0; i < period - 1; ++i) {
        maxs.add(0);
        mins.add(0);
        modifiedSource.add(DataPoint(x:data[i].datetime,y:data[i].close, stock:data[i],index:i));
      }
      num min, max;
      ...
      for (int i = period - 1; i < data.length; ++i) {
        ...
        modifiedSource.add(DataPoint(x:data[i].datetime,y:(top / bottom) * 100,stock:data[i],index:i
            ));
      }
    }
 ...
 static List<DataPoint> sma(int period, List<Stock> stocks, String tipocalculo){
      ...
      for (int i = 0; i < limit; i++) {    
        ...
        if (i >= length - 1 && i < limit) {
          if (i - period >= 0) {
            ...
            smaPoints[i] = DataPoint(x:stocks[i].datetime,y:sma,stock:stocks[i],index:i);
          } else {
            smaPoints[i] = DataPoint(x:stocks[i].datetime,y:sma,stock:stocks[i],index:i);
      ...
      for (int i = 0; i < limit; i++) {    
        ...
        if (i >= length - 1 && i < limit) {
          if (i - period >= 0) {
            ...
            smaPoints[i] = DataPoint(x:datapoint[i].x,y:sma,stock:null,index:i);
          } else {
            smaPoints[i] = DataPoint(x:datapoint[i].x,y:0,stock:null,index:i);
        ...
        // print("x:${datapoint[i].x}, y:$sum,stock:${datapoint[i]},index: $i");
      }
...
static Map<String,Map<String,dynamic>> generateMapSignals(List<IndicadoresOpcao> signals, List<Stock> stocks, String tipo){
        ...
        switch (indicadoropcao.id) {
          case 1:
            {
              ...
              if(!(mapsignals["mapAcumDist"].containsKey(key))){
                mapsignals["mapAcumDist"][key] = GenerateDataPoint.accumulationdistribution(stocks);
                print("criando datapoints mapAcumDist no periodo $key $tipo");
                if(indicadoropcao.configselecionado.id == "acudis5" || indicadoropcao.configselecionado.id == "acudis6"){
                  ...
                  if(!(mapsignals["mapAcumDist"].containsKey(keymodified))){
                    ...
                    mapsignals["mapAcumDist"][keymodified] = GenerateDataPoint.smaIndicator(periodocalculo, mapsignals["mapAcumDist"][key]);
                    // for (int i = 0; i <  mapAcumDist[keymodified].length; i++) {
                    //   print("Valor atual:${mapAcumDist[key][i].y} /// Média:${mapAcumDist[keymodified][i].y}");
 
                    // }
                   
                    print("criando media mapAcumDist no periodo $keymodified $tipo");
                  }
          ...
          case 3:
            {
             ...
             if(!(mapsignals["mapBollinger"].containsKey(key))){
                ...
                mapsignals["mapBollinger"][key] = GenerateDataPoint.bollinger(desvio,periodo, stocks);
                print("criando datapoints mapBollinger com periodo $key no $tipo");
              } 
            }
            break;
          case 4:
          {
            ...
            if(!(mapsignals["mapEma"].containsKey(key))){
                mapsignals["mapEma"][key] = GenerateDataPoint.ema(periodo, stocks);
                print("criando datapoints mapEma com periodo $key no $tipo");

                if(indicadoropcao.configselecionado.id == "ema7" || indicadoropcao.configselecionado.id == "ema8"){
                  ...
                  if(!(mapsignals["mapEma"].containsKey(keymodified))){
                    mapsignals["mapEma"][keymodified] = GenerateDataPoint.ema(periodolongo, stocks);
                  }
                }
            }
          }
          ...
          case 5:
          {
            ...
            if(!(mapsignals["mapMacd"].containsKey(key))){
                ...
                mapsignals["mapMacd"][key] = GenerateDataPoint.macd(fastPeriod,slowPeriod,period, stocks);
          ...
          case 7:
          {
            ...
            if(!(mapsignals["mapIfr"].containsKey(key))){
                mapsignals["mapIfr"][key] = GenerateDataPoint.ifr(indicadoropcao.parametro["periodo"], stocks);
          ...
          case 8:
          {
            ...
            if(!(mapsignals["mapSma"].containsKey(key))){
                mapsignals["mapSma"][key] = GenerateDataPoint.sma(periodo, stocks, "close");
                print("criando datapoints mapSma com periodo $key no $tipo");

                if(indicadoropcao.configselecionado.id == "media7" || indicadoropcao.configselecionado.id == "media8"){
                  num periodolongo = indicadoropcao.parametro["periodolongo"];
                  String keymodified = "$id:$periodo:$periodolongo";
                  if(!(mapsignals["mapSma"].containsKey(keymodified))){
                    mapsignals["mapSma"][keymodified] = GenerateDataPoint.sma(periodolongo, stocks, "close");
                    print("criando datapoints mapSma medialonga com periodo $keymodified no $tipo");
                  }
                }
            }
          ...
          case 9:
          {
              ...
              //////////Source = dados formatados para datapoint
              //////////periodCollection = posiçoes em y da linha amarela -> mapStochastic[key]["periodCollection"][i].y}
              //////////periodX = posiçoes em x da linha amarela mapStochastic[key]["periodCollection"][i]}
              //////////signalCollection = posiçoes em y da linha azul -> mapStochastic[key]["signalCollection"][i].y}
              //////////signalX = posiçoes em y da linha azul ->mapStochastic[key]["periodCollection"][i]}
              int periodo = indicadoropcao.parametro["periodo"];
              if(!(mapsignals["mapStochastic"].containsKey(key))){
                  ...
                  mapsignals["mapStochastic"][key]["periodCollection"] = <DataPoint>[];
                  mapsignals["mapStochastic"][key]["signalCollection"] = <DataPoint>[];

  
                  // List<Stock> data = stocks.where((e) => e.datetime.year == 2019 && e.datetime.month > 1 && e.datetime.month < 4).toList();
                 
                  source = GenerateDataPoint.calculatePeriod(
                    periodo,
                    indicadoropcao.parametro["k"],
                    stocks
                );
                  ...
                  print("criando datapoints mapStochastic com periodo $key no $tipo");
          ...
          case 12:
            {
              ...
              if(!(mapsignals["mapEma"].containsKey(keymodified))){
                  mapsignals["mapEma"][keymodified] = GenerateDataPoint.emabody(c_Len, stocks);
                  print("criando datapoints mapEma com periodo $keymodified no $tipo");
              } 
          ...
          case 13:
            {
               List<IndicadoresOpcao> indicadoresopcao = indicadoropcao.indicadoresopcoes;
               Map<String,Map<String,dynamic>> multimap = GeraMapDataPoint.generateMapSignals(indicadoresopcao, stocks, "MultiCondicional");
          ...
          case 14:
            {
              ...
              if(indicadoropcao.configselecionado.id == "saida5" || indicadoropcao.configselecionado.id == "saida6"){
                  String keymodified = "$id:$periodo:atr";
                  if(!(mapsignals["mapAtr"].containsKey(keymodified))){
                    mapsignals["mapAtr"][keymodified] = GenerateDataPoint.atr(indicadoropcao.parametro["periodo"], stocks);
                    print("criando datapoints mapAtr com periodo $keymodified no $tipo");
                  }
              }else if(indicadoropcao.configselecionado.id == "saida7"){
                  String keymodified = "$id:$periodo:close";
                  if(!(mapsignals["mapSma"].containsKey(keymodified))){
                    mapsignals["mapSma"][keymodified] = GenerateDataPoint.sma(indicadoropcao.parametro["periodo"], stocks, "close");
                    print("criando datapoints mapSma com periodo $keymodified no $tipo");
                  }
              }else if(indicadoropcao.configselecionado.id == "saida8"){
                  String keymodified = "$id:$periodo:high";
                  if(!(mapsignals["mapSma"].containsKey(keymodified))){
                    mapsignals["mapSma"][keymodified] = GenerateDataPoint.sma(indicadoropcao.parametro["periodo"], stocks, "high");
                    print("criando datapoints mapSma com periodo $keymodified no $tipo");
                  }
              }else if(indicadoropcao.configselecionado.id == "saida9"){
                  String keymodified = "$id:$periodo:low";
                  if(!(mapsignals["mapSma"].containsKey(keymodified))){
                    mapsignals["mapSma"][keymodified] = GenerateDataPoint.sma(indicadoropcao.parametro["periodo"], stocks, "low");
                    print("criando datapoints mapSma com periodo $keymodified no $tipo");
                  }
              }
...
static MapIndicadores generateMap(List<IndicadoresOpcao> buysignals, List<IndicadoresOpcao> sellsignals, stocks){



 MapIndicadores map = MapIndicadores(mapSma: {}, mapStochastic: {}, mapIfr: {}, mapMacd: {}, mapBollinger: {}, mapAcumDist: {}, mapEma:{}, mapAtr:{});
 ...
 Map<String,Map<String,dynamic>> mapsignals = {
  ...
  Map<String,Map<String,dynamic>> buymap = generateMapSignals(buysignals,stocks,"buysignals");
  Map<String,Map<String,dynamic>> sellmap = generateMapSignals(sellsignals,stocks,"sellsignals");
  ...
  map = MapIndicadores(mapSma:mapsignals["mapSma"],mapStochastic:mapsignals["mapStochastic"],mapIfr: mapsignals["mapIfr"], mapMacd: mapsignals["mapMacd"], mapBollinger:mapsignals["mapBollinger"], mapAcumDist:mapsignals["mapAcumDist"], mapEma:mapsignals["mapEma"], mapAtr:mapsignals["mapAtr"]);
  return map;
