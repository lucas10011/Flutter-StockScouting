...
class TendenciaPage extends StatefulWidget {
  ...
  @override
  _TendenciaPageState createState() => _TendenciaPageState(stockname: stockname,stocks: stocks);
}
...
class _TendenciaPageState extends State<TendenciaPage> {
 ...
 @override
  void initState() {
    ...
    List<Stock> filtered = stocks.where((e) => e.datetime.year == stocks[0].datetime.year).toList();
...
double getmax(List<Stock> candles){
  ...
  Future<List<Stock>> generateDataPorAno(List<Stock> candles,bool v) async {
     ...
     try { 
      ...
      double mediavariacaonominal = v ? roundDouble((variacaonominal / candles.length), 2 ) : 0.0;
      double basemedia = mediavariacaonominal;

      List<Stock> list = candles
          .map((Stock stock){
        ...
        builder: (BuildContext context) => Container(
              ...
              child: YearPicker(
                ...
                onChanged: (val) {
                  ...
                  setState(() {
                    dtstart = val;
                    List<Stock> filtered = stocks.where((e) => e.datetime.year == val.year).toList();
                    stocksporano = generateDataPorAno(filtered,variacao);
                  });
...
_buildPageMain(BuildContext context, List<Stock> candles){
              ...
              return Column(
                ...
                children: [
                  ...
                  Text("${dtstart.year}", style: TextStyle(fontSize: 30,color: Colors.white),),
